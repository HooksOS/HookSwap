// v4 sub-adapter — STUB (documented). HookSwap is supportsV4:false today
...
// Settlement.sol — only this file + a route entry.
//
...
//    observe()); e.g. a Truncated/Geomean-oracle hook. So a v4 route needs BOTH
...
//  1. Extend OracleRoute with { stateView, poolId, oracleHook? } (or reuse
//     poolAddress = StateView and add poolId).
...
export class V4Adapter implements ISubAdapter {
  ...
  async price(route: OracleRoute): Promise<MarkPrice> {
    return {
      price1e18: 0n,
      ok: false,
      source: `${route.protocol}:${route.poolAddress}`,
      reason:
        "V4_NOT_IMPLEMENTED: HookSwap is supportsV4:false. v4 is a singleton PoolManager " +
        "(read via StateView.getSlot0(poolId)); needs {stateView,poolId,oracleHook?} route fields. " +
