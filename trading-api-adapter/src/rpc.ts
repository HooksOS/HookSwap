/**
 * Multi-RPC auto-failover for the HookSwap Trading API adapter.
 *
 ...
 *
 * CONTRACT (identical in data-api/src/rpc.ts — the two services are deployed separately, so
 ...
 *
 * The adapter's own code path does NOT call eth_getLogs (routing is multicall/quoter reads only), so
 * nothing here consumes this today. It is exported to keep this module byte-identical in shape with
 * data-api/src/rpc.ts (which DOES scan logs) and so any future log scan added here inherits the safe
 * bound rather than re-deriving one.
 *
 * Caps measured live 2026-08-02: XLayer's `rpc.xlayer.tech` / `xlayerrpc.okx.com` = 100 blocks,
 * Stable's `rpc.stable.xyz` / `stable.drpc.org` = 500, HyperEVM's `hyperliquid.rpc.blxrbdn.com` = 1000;
 * drpc free plans refuse >10000 and additionally cap RESULT COUNT at 20000 logs. Three of those are
 * BELOW 2000, so any log scan must ALSO bisect on error (as data-api's onchain.getLogsBisecting /
 * indexer.getLogsAdaptive do) rather than rely on this constant alone.
 */
 ...
 * inside FailoverProvider.send so EVERY caller is covered rather than each re-deriving its own limit.
 * In THIS service the meter is present but DISABLED by default (see DEFAULT_GET_LOGS_BUDGET) because it
 * runs no scans; data-api's copy is where the Robinhood budget is actually enforced. Cheap foreground reads
...
/**
 * Fallback `eth_getLogs` budget per window. **0 = DISABLED, and that is the adapter's deliberate default.**
 *
 * DIVERGENCE FROM data-api (the only difference between the two copies of this module — the code and the
 * exported API are identical, only these two default tables differ):
 *   - data-api SCANS logs (pool discovery + the event indexer). It is the service that exhausted
 *     Robinhood's budget, so there the defaults are 60/window generally and 8/window on 4663.
 *   - THIS service does not scan: routing is multicall/quoter reads. It is FOREGROUND — a user is
 *     waiting on every call. And its provider is handed to the embedded smart-order-router
 *     (embedRouter.ts), so it has NOT been verified here that no SOR code path emits eth_getLogs.
 *     Metering by default could therefore turn a live quote into an error to solve a problem this
 *     service does not have. Opt in per chain with `GETLOGS_BUDGET_<chainId>` if that ever changes.
 */
export const DEFAULT_GET_LOGS_BUDGET = 0

/**
 * Per-chain `eth_getLogs` budget overrides (calls per GET_LOGS_WINDOW_MS).
 *
 * EMPTY here on purpose — see DEFAULT_GET_LOGS_BUDGET above for why this service does not meter by
 * default. data-api's copy carries `{ 4663: 8 }`, which is where the Robinhood limit actually bites.
 */
...
export class FailoverProvider extends ethers.providers.StaticJsonRpcProvider {
  ...
  async send(method: string, params: Array<any>): Promise<any> {
    // METER `eth_getLogs` ONLY — every other method (eth_call, eth_blockNumber, …) is a cheap foreground
    // read and must never be blocked by a log budget. No-op unless a budget is configured for the chain
    // (this service defaults to DISABLED — see DEFAULT_GET_LOGS_BUDGET).
...
/**
 * Build the provider for a chain: a plain StaticJsonRpcProvider when there is exactly one endpoint,
 * a FailoverProvider otherwise. Return type is StaticJsonRpcProvider so callers that need the
...
export function createFailoverProvider(
  ...
  // bare StaticJsonRpcProvider, which bypasses `send` and therefore the eth_getLogs meter. Robinhood
  // (4663) has exactly ONE working public RPC, so the shortcut left precisely the chain that needs
  // metering unmetered. With one url the failover loop is a single attempt — behaviourally identical.
