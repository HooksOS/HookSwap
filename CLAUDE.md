...
- **✅ SELF-SERVICE MARKET FACTORY — Sepolia deployed + on-chain tested (2026-07-19).** Projects launch their own isolated perp market in one tx via `PerpMarketFactory.createMarket` (EIP-1167 minimal-proxy clones of an initializable `PerpMarket` = clone-safe Settlement). Platform earns via `FeeRouter`: every trade's per-side fee splits **platform 50% (hard floor 40%) / creator 40% / insurance 10%**, non-bypassable (feeReceiver=FeeRouter set before ownership → platformAdmin; creator only earns). Sepolia addrs + full proof in `contracts/perps/config/factory-sepolia.json` (Factory `0x5F3803B0…`, FeeRouter `0x493A120c…`, Registry `0xEDE27846…`, impl `0xc42F9290…`). On-chain assertions PASS: market isolation (marketB fee balance 0 while marketA trades), platform floor share, creator share, creatorB=0, insurance 10% — 18 txs all status 0x1. Design: `contracts/perps/SELF_SERVICE_SPEC.md`. Dep: openzeppelin-contracts-upgradeable@v5.0.2. **v1 limits:** per-market leverage cap stored/off-chain-enforced (PerpMarket keeps the 100x constant → v2 ParamGuard); oracle-safety guards, slashable bonds, per-market insurance sub-accounts = spec'd, not built; full audit + non-throwaway platformAdmin/matcher/treasury before mainnet.
