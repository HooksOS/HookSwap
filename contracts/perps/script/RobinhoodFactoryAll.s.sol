...
/// @dev Minimal Chainlink aggregator surface for reading the live Robinhood ETH/USD reference.
interface IAgg {
...
/**
 * @title RobinhoodFactoryAll
 * @notice USER-AUTHORIZED MAINNET PILOT deploy of the HookSwapPerps self-service FACTORY stack
 *         on Robinhood Chain (chainId 4663). Faithful adaptation of script/SepoliaFactoryAll.s.sol:
 *
 *           * WETH  = Robinhood canonical wrapped-native 0x0Bd7D308…EAcAD73.
 *           * feed  = LIVE, on-chain-verified Chainlink ETH/USD proxy on RH
 *                     0x78F3556b67E17Df817D51Ef5a990cDaF09E8d3A9 (8 decimals, "ETH / USD").
 *           * fee sink / slash sink / owner of every stack contract = TREASURY 0x011d438E…6529E13.
 *           * matcher for every created market                      = the DEPLOYER 0xc14C… (pilot;
 *                     a real perps-engine matcher gets wired later).
 *
 *         Model (mirrors Sepolia): the deploying EOA temporarily holds platformAdmin/owner/matcher
 *         so it can (a) deploy + wire the fresh stack and (b) drive EVERY proof in ONE broadcast.
 *         AFTER the proofs, control is HANDED to the treasury: factory.setPlatformWiring routes all
 *         FUTURE markets to platformAdmin=TREASURY (matcher stays the deployer), then Ownable
 *         ownership of every stack contract AND the 5 proof markets → TREASURY.
 *
 *         Economics are scaled down 1000× vs Sepolia (MATCH_SIZE 1e13, DEP 3e12) purely to keep the
 *         real-money collateral/bond outlay tiny; every code path / proof is identical, just smaller.
 *
 *         Features proven (PROOF labels), each with a `require` + console2.log:
 *           1  createMarket           — clone registered ACTIVE + bond escrowed + settle path wired
 ...
 *           H  Handover               — platformAdmin + every owner() == TREASURY
 *
 * Broadcast (operator, funded key):
 *   forge script script/RobinhoodFactoryAll.s.sol \
 *     --rpc-url https://rpc.mainnet.chain.robinhood.com \
 *     --broadcast --private-key <PK> --slow
 */
contract RobinhoodFactoryAll is Script {
    // ---- Robinhood canonical addresses (WETH9 + LIVE, on-chain-verified Chainlink ETH/USD) ----
    address internal constant WETH = 0x0Bd7D308f8E1639FAb988df18A8011f41EAcAD73;
    address internal constant ETH_USD = 0x78F3556b67E17Df817D51Ef5a990cDaF09E8d3A9;

    // ---- roles ----
    address internal constant TREASURY = 0x011d438E3eb3fce848950859591ec037C6529E13;
    ...
    // ---- economics (scaled 1000x down vs Sepolia to keep the pilot outlay tiny) ----
    uint256 internal constant MATCH_SIZE = 1e13; // notional per side
    uint256 internal constant FEE_RATE = 10; // 0.10% per side → perSideFee = 1e10
    uint256 internal constant PER_SIDE_FEE = (MATCH_SIZE * FEE_RATE) / 10000; // 1e10
    uint256 internal constant FEE_ACCRUED = PER_SIDE_FEE * 2; // 2e10 to feeReceiver
    uint256 internal constant DEP = 3e12; // per-trader per-market collateral deposit
    uint256 internal constant COVER_AMT = 1e9; // insurance coverLoss test amount
    ...
    function run() external {
        address deployer = msg.sender; // = platformAdmin(temp) = owner(temp) = matcher
        ...
        console2.log("=== HookSwapPerps FACTORY mainnet PILOT (Robinhood 4663) ===");
        console2.log("chainId :", block.chainid);
        console2.log("deployer/matcher (temp owner during proofs):", deployer);
        console2.log("TREASURY (final owner/admin/fee+slash sink)  :", TREASURY);
        require(block.chainid == 4663, "not Robinhood (4663)");
        ...
        console2.log("live RH ETH/USD ref (1e18):", refPrice);
        ...
        // ---- FEATURE H: hand every owner/admin to the treasury (matcher stays deployer) ----
        _handover(deployer);
        ...
        console2.log("=== ALL FACTORY FEATURES PROVEN ON-CHAIN; OWNERSHIP -> TREASURY ===");
    }

    // ============================================================
    // Deploy + wire a FRESH factory-v4 stack (deployer temporarily owns everything)
    // ============================================================
    function _deployStack(address deployer) internal {
        ...
        // BondManager slash sink = TREASURY (task: bond-slash-recipient = treasury).
        bondManager = new BondManager(TREASURY, address(registry));
        // FeeRouter platform treasury = TREASURY (platform share is claim-based storage).
        feeRouter = new FeeRouter(TREASURY, address(insuranceHub));
        impl = new PerpMarket();
        factory = new PerpMarketFactory(
            address(impl),
            WETH,
            deployer, // platformMatcher (authorized matcher on every market clone) = deployer (pilot)
            deployer, // platformAdmin (temp; handed to TREASURY after proofs)
            address(feeRouter),
            address(registry),
            deployer, // listing-fee treasury (temp; handed to TREASURY after proofs)
        ...
        insuranceHub.setAuthorizedCoverer(msg.sender, true);
    ...
    function _assertFeeSplitAndInsurance() internal {
        ...
        require(availA == FEE_ACCRUED, "F2: marketA fee accrual != 2e10");

        uint256 collected = feeRouter.collect(marketA);
        require(collected == FEE_ACCRUED, "F2: collected != 2e10");

        uint256 platform = feeRouter.claimable(TREASURY, WETH); // platform treasury == TREASURY
        uint256 creatorCut = feeRouter.claimable(creatorA, WETH);
        uint256 insurance = insuranceHub.balanceOf(marketA, WETH);
        require(platform == 1e10, "F2: platform != 50%");
        require((platform * 10000) / collected >= 4000, "F2: below 40% floor");
        require(creatorCut == 8e9, "F2: creatorA != 40%");
        require(insurance == 2e9, "F2: insurance != 10%");
        require(feeRouter.claimable(creatorB, WETH) == 0, "F2: creatorB earned from A");
        require(IERC20(WETH).balanceOf(address(insuranceHub)) == 2e9, "F5: hub WETH != 2e9");
    ...
    // ============================================================
    function _coverLossSuccess() internal {
        ...
        require(insuranceHub.balanceOf(marketA, WETH) == 2e9 - COVER_AMT, "F5a: A sub-account not debited");
    ...
    function _slashBond() internal {
        uint256 sinkBefore = TREASURY.balance;
        uint256 escrowBefore = address(bondManager).balance;
        bondManager.slash(marketB);
        require(TREASURY.balance == sinkBefore + BOND, "F4b: treasury not credited");
        ...
        console2.log("PROOF 4b slash: marketB bond -> treasury delta =", TREASURY.balance - sinkBefore, "| escrow now =", address(bondManager).balance);
    }

    // ============================================================
    // FEATURE H — hand every owner/admin to the treasury (matcher stays the deployer)
    // ============================================================
    function _handover(address deployer) internal {
        // Route ALL future markets to platformAdmin = TREASURY; matcher stays the deployer (pilot).
        factory.setPlatformWiring(
            WETH,
            deployer, // platformMatcher stays the deployer
            TREASURY, // platformAdmin → treasury (future markets owned by treasury)
            address(feeRouter),
            address(registry),
            TREASURY, // listing-fee treasury → treasury
            0 // listingFee unchanged
        );

        // Transfer Ownable ownership of every stack contract → TREASURY.
        registry.transferOwnership(TREASURY);
        oracleGuard.transferOwnership(TREASURY);
        paramGuard.transferOwnership(TREASURY);
        insuranceHub.transferOwnership(TREASURY);
        bondManager.transferOwnership(TREASURY);
        feeRouter.transferOwnership(TREASURY);
        factory.transferOwnership(TREASURY);

        // Transfer the 5 proof markets → TREASURY (they were minted deployer-owned).
        PerpMarket(payable(marketA)).transferOwnership(TREASURY);
        PerpMarket(payable(marketB)).transferOwnership(TREASURY);
        PerpMarket(payable(marketLev)).transferOwnership(TREASURY);
        PerpMarket(payable(marketFallback)).transferOwnership(TREASURY);
        PerpMarket(payable(staleMarket)).transferOwnership(TREASURY);

        // Verify on-chain: platformAdmin + every owner() == TREASURY.
        require(factory.platformAdmin() == TREASURY, "H: factory.platformAdmin != treasury");
        require(factory.owner() == TREASURY, "H: factory.owner != treasury");
        require(registry.owner() == TREASURY, "H: registry.owner != treasury");
        require(oracleGuard.owner() == TREASURY, "H: oracleGuard.owner != treasury");
        require(paramGuard.owner() == TREASURY, "H: paramGuard.owner != treasury");
        require(insuranceHub.owner() == TREASURY, "H: insuranceHub.owner != treasury");
        require(bondManager.owner() == TREASURY, "H: bondManager.owner != treasury");
        require(feeRouter.owner() == TREASURY, "H: feeRouter.owner != treasury");
        require(PerpMarket(payable(marketA)).owner() == TREASURY, "H: marketA.owner != treasury");
        require(factory.platformMatcher() == deployer, "H: matcher != deployer");
        console2.log("PROOF H handover: platformAdmin + all stack/market owners = TREASURY; matcher = deployer");
    }

    // ============================================================
    // FEATURE 6 — OracleGuard: deviation PASS + deviation / staleness / venue-allowlist reverts
    // ============================================================
    function _assertOracleGuard() internal {
    ...
    // ============================================================
    // FEATURE 7 — ParamGuard leverage + fee bounds (check/clamp)
    // ============================================================
